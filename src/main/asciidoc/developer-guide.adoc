...
The format of JSON objects in requests and responses has been omitted in order 
to keep the listing short. Whenever a Command or Payload column indicates a 
type, like `VerifyEmailAddress` or `CredentialsDTO`, the corresponding JSON 
object format can be deduced from inspecting the similar JAVA class by 
translating any getter method to a corresponding JSON property. Similarly, any 
query responses can be one of:
- a single ´Entry´ object or ´DTO´ object 
- a list of ´Entry´ objects
- a "paged" object (see section "paged result") with a content of a list of 
´Entry´ objects
