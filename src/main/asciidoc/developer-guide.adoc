...
=== APPENDIX A: The REST interface
The Maritime Cloud Portal REST interface uses Jersey JAX-RS 
[https://jersey.java.net/]. 

The REST solution is mainly inspired by this article [http://www.infoq.com/articles/rest-api-on-cqrs].
...
payload of a JSON formatted command. 
...
by the envisioned "Maritime Almanac", although the realization of the maratime 
...
curl -c cookies.txt http://localhost:8080/rest/authentication/login -H "Content-Type: application/json" -d '{"username":"admin","password":"test"}'
...
1) Command-name may optionally be suffixed with "Command" like "RegisterUserCommand".

5+a| Examples:
curl http://localhost:8080/rest/api/users -H "Content-Type: application/json;domain-model=RegisterUser" -d 
  '{
     "userId":{"identifier":"9acd9a23-ab23-44a7-8203-18e700ea0da1"},
